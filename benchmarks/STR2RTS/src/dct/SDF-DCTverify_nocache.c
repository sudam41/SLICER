#include "SDF-DCTverify_nocache.h"
...
void AnonFilter_a0_239() {
	FOR(int, i, 0,  < , 64, i++) {
		push_int(&AnonFilter_a0_239DUPLICATE_Splitter_248, (((int) pow(3.0, i)) % 75)) ; 
	}
	ENDFOR
}
...
void DCT_2D_reference_coarse_243() {
	...
	FOR(int, i, 0,  < , 8, i++) {
		FOR(int, j, 0,  < , 8, j++) {
			block_x[i][j] = 0.0 ; 
			FOR(int, k, 0,  < , 8, k++) {
				block_x[i][j] = (block_x[i][j] + (DCT_2D_reference_coarse_243_s.coeff[j][k] * peek_int(&SplitJoin0_SplitJoin0_SplitJoin0_AnonFilter_a2_240_250_253_256_split[0], ((8 * i) + k)))) ; 
			}
			ENDFOR
		}
		ENDFOR
	...
	FOR(int, i, 0,  < , 8, i++) {
		FOR(int, j, 0,  < , 8, j++) {
			...
			push_int(&DCT_2D_reference_coarse_243iDCT_2D_reference_coarse_245, ((int) block_y)) ; 
		}
		ENDFOR
	}
	ENDFOR
	FOR(int, i, 0,  < , 64, i++) {
		pop_int(&SplitJoin0_SplitJoin0_SplitJoin0_AnonFilter_a2_240_250_253_256_split[0]) ; 
	}
	ENDFOR
}
...
void iDCT_2D_reference_coarse_245() {
	...
	FOR(int, i, 0,  < , 8, i++) {
		FOR(int, j, 0,  < , 8, j++) {
			block_x[i][j] = 0.0 ; 
			FOR(int, k, 0,  < , 8, k++) {
				block_x[i][j] = (block_x[i][j] + (iDCT_2D_reference_coarse_245_s.coeff[k][j] * peek_int(&DCT_2D_reference_coarse_243iDCT_2D_reference_coarse_245, ((8 * i) + k)))) ; 
			}
			ENDFOR
		}
		ENDFOR
	...
	FOR(int, i, 0,  < , 8, i++) {
		FOR(int, j, 0,  < , 8, j++) {
			...
			ENDFOR
			block_y = ((float) floor((block_y + 0.5))) ; 
			push_int(&SplitJoin0_SplitJoin0_SplitJoin0_AnonFilter_a2_240_250_253_256_join[0], ((int) block_y)) ; 
		}
		ENDFOR
	}
	ENDFOR
	FOR(int, i, 0,  < , 64, i++) {
		pop_int(&DCT_2D_reference_coarse_243iDCT_2D_reference_coarse_245) ; 
	}
	ENDFOR
}
...
void Identity_246(){
	FOR(uint32_t, __iter_steady_, 0, <, 64, __iter_steady_++) {
		int __tmp3 = 0;
		__tmp3 = pop_int(&SplitJoin0_SplitJoin0_SplitJoin0_AnonFilter_a2_240_250_253_256_split[1]) ; 
		push_int(&SplitJoin0_SplitJoin0_SplitJoin0_AnonFilter_a2_240_250_253_256_join[1], __tmp3) ; 
	}
	ENDFOR
}
...
void AnonFilter_a3_247(){
	FOR(uint32_t, __iter_steady_, 0, <, 64, __iter_steady_++) {
		int a = 0;
		int b = 0;
		a = pop_int(&WEIGHTED_ROUND_ROBIN_Joiner_249AnonFilter_a3_247) ; 
		b = pop_int(&WEIGHTED_ROUND_ROBIN_Joiner_249AnonFilter_a3_247) ; 
