...
data4 = pd.read_csv('fail4.csv', sep=",")
data5 = pd.read_csv('fail5.csv', sep=",")
...
fail4 = np.array(data4,dtype=float)
fail5 = np.array(data5,dtype=float)
...
for j,fail in enumerate(fails):
	for i,core in enumerate(fail[:6]):
		ax[int(i/2),int(i%2)].plot(core[0:3000],label="F{}".format(j))
